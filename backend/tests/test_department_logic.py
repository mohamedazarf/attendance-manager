...
def test_department_logic():
    ...
    # Logs for an employee starting at 7:45 AM (late for Usine 7:30, on time for Admin 8:30)
    # Ends at 4:45 PM (on time for Usine 4:30, early for Admin 5:30)
    raw_logs = [
        {"user_id": 1, "timestamp": datetime.combine(test_date, time(7, 45))},
        {"user_id": 1, "timestamp": datetime.combine(test_date, time(16, 45))}
    ]
    
    print("Testing Usine Department (Target: 07:30 - 16:30)")
    processed_employee = processor._process_daily_logs(1, test_date, raw_logs, department="usine")
    ...
    # Assertions
    assert processed_employee.is_late == True, "Usine should be late"
    ...
    assert "EARLY_DEPARTURE" not in processed_employee.anomalies, "Usine should NOT have early departure anomaly"
