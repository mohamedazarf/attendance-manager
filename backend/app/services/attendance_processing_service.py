...
class AttendanceProcessingService:
    ...
    def process_logs(
        self, logs: List[dict], user_departments: Dict[int, str] = None
    ) -> Dict[Tuple[int, date], ProcessedAttendance]:
        """
    ...
    def _process_daily_logs(
        ...
        processed = ProcessedAttendance(user_id=user_id, date=log_date, events=events)
    ...
    def _get_effective_department_schedule(
        ...
        start/end time and pause.
        """
        try:
            return self.day_rules_service.get_department_schedule_for_date(
                processed.date, department
            )
        except Exception:
            dept_cfg = self.config.get_department_config(department)
            return {
                "start_time": dept_cfg["start_time"],
                "end_time": dept_cfg["end_time"],
                "pause_minutes": self.config.PAUSE_DURATION,
                "source": "default",
            }
    ...
    def calculate_hours(self, processed: ProcessedAttendance, department: str = "employee"):
        ...
        schedule = self._get_effective_department_schedule(processed, department)
        pause_hours = float(schedule.get("pause_minutes", self.config.PAUSE_DURATION)) / 60
        ...
        total_seconds_expected = (end.hour * 3600 + end.minute * 60) - (
    ...
    def _detect_anomalies(self, processed: ProcessedAttendance, department: str = "employee"):
        ...
        schedule = self._get_effective_department_schedule(processed, department)
        dept_start_time = schedule["start_time"]
        dept_end_time = schedule["end_time"]
        ...
        if processed.check_in_time:
            ...
            start_with_tolerance = start_time_utc + timedelta(
                minutes=self.config.LATE_TOLERANCE
            )
        ...
        processed.is_complete_day = bool(
            processed.check_in_time and processed.check_out_time
        )
        ...
        if anomalies:
            processed.has_anomaly = True
            processed.anomalies = list(set(anomalies))
            processed.status = "anomaly"
        else:
            processed.status = "normal"

    def generate_summary(
        self, processed: ProcessedAttendance, employee_info: dict = None
    ) -> DailyAttendanceSummary:
        """
