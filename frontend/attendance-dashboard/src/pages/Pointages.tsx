...
//       boxShadow: onClick ? "lg" : undefined,
//     }}
//     onClick={onClick}
//   >
//     <Text fontSize="sm" color="gray.600" mb={2}>
...
//   /* -------------------- Fetch data -------------------- */
//   useEffect(() => {
//     const TEST_DATE = "2026-01-26"; // change date if needed
...
//   /* -------------------- Sync selectedFilter with URL -------------------- */
//   useEffect(() => {
//     const filter = new URLSearchParams(location.search).get("filter") as
//       | "all"
//       | "present"
//       | "absent"
//       | null;
...
//                     navigate("/employeesToday?filter=all");
//                   }}
//                 />
//                 <StatCard
//                   label="Present Today"
...
//                 {dashboard?.employees
//                   .filter((emp) => !emp.check_in_time || !emp.check_out_time)
//                   .map((emp) => (
//                     <Box
//                       key={emp.employee_id}
//                       p={4}
...
//                       <Badge colorScheme="red">Missing Punch</Badge>
//                     </Box>
//                   ))}
...
//                   (emp) => emp.check_in_time && emp.check_out_time
//                 ).length === 0 && (
//                   <Text color="gray.500">No alerts today</Text>
//                 )}
//               </Box>
//             </>
//           )}
...
const StatCard = ({
  ...
  <Box
    ...
    _hover={
      onClick
        ? { borderColor: "blue.400", transform: "translateY(-2px)", boxShadow: "lg" }
        : undefined
    }
    onClick={onClick}
  >
            ...
            <>
              ...
              <SimpleGrid columns={{ base: 1, md: 4 }} spacing={6} mb={8}>
                <StatCard
                  label="Total Employees"
                  value={dashboard?.global.total_employees ?? 0}
                  onClick={() => navigate("/employeesToday?filter=all")}
                />
                <StatCard
                  label="Present Today"
                  value={dashboard?.global.present_today ?? 0}
              ...
              <Box bg="white" p={6} borderRadius="lg" boxShadow="sm">
                ...
                {dashboard?.employees.filter(
                  (emp) => emp.check_in_time && emp.check_out_time
                ).length === 0 && <Text color="gray.500">No alerts today</Text>}
              </Box>
            </>
          )}
        </Container>
