...
export const ManualPunchModal: React.FC<ManualPunchModalProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(t("Failed to record manual punch"));
      }
    } catch (error: any) {
      toast({
        title: t("Error"),
        description: error.message || t("Failed to record manual punch"),
        status: "error",
        duration: 3000,
      });
    } finally {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      ...
      <ModalContent>
        <ModalHeader>
          {t("Manual punch for")} {employee?.name}
        </ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          <FormControl mb={4}>
...
export const MarkAbsentModal: React.FC<MarkAbsentModalProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(t("Failed to confirm absence"));
      }
    } catch (error: any) {
      toast({
        title: t("Error"),
        description: error.message || t("Failed to confirm absence"),
        status: "error",
        duration: 3000,
      });
    } finally {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      ...
      <ModalContent>
        <ModalHeader>
          {t("Confirm absence for")} {employee?.name}
        </ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          <FormControl mb={4}>
            ...
            <Select value={reason} onChange={(e) => setReason(e.target.value)}>
              ...
              <option value="Absence injustifiée">
                {t("Unjustified Absence")}
              </option>
